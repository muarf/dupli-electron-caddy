...
if (isset($argv[0]) && realpath($argv[0]) === realpath(__FILE__)) {
    ...
    $arg = $argv[1] ?? '';
    if (file_exists($arg) && (str_ends_with($arg, '.json') || strpos($arg, 'pdf_tool_payload') !== false)) {
        $payloadStr = file_get_contents($arg);
        @unlink($arg);
    } else {
        $payloadStr = $arg;
    }
    ...
    if (!is_array($payload)) {
        fwrite(STDERR, "Payload invalide: " . var_export($payloadStr ?? null, true) . " - Error: " . json_last_error_msg() . "\n");
        exit(1);
    }
