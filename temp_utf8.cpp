...
// ---------------------------------------------------------------------------
// Logging minimal ÔÇö chemin dynamique via %LOCALAPPDATA%
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// D├®tection du format SPL (lecture des premiers octets uniquement)
// ---------------------------------------------------------------------------
...
std::string MonitorWorker::FindSplPath(uint32_t jobId) {
  ...
  // 3. Fallback : SPL FP le plus r├®cent
...
JobEvent MonitorWorker::GetJobEvent(void* hPrinter, uint32_t jobId) {
  ...
  // SPL path + format (lecture disque l├®g├¿re, seulement 256 octets)
...
// ---------------------------------------------------------------------------
// Thread de monitoring ÔÇö event-driven via FindFirstPrinterChangeNotification
// Fallback polling 500ms si la notification ├®choue (imprimante r├®seau, etc.)
// ---------------------------------------------------------------------------
void MonitorWorker::Execute(const ExecutionProgress& progress) {
  // jobKey ÔåÆ ├®tat courant (pour ne notifier que les changements)
  std::map<std::string, std::string> seenStates;

  // Active "Keep Printed Jobs" une seule fois au d├®marrage
...
// ---------------------------------------------------------------------------
// Callback N-API ÔÇö conversion JobEvent ÔåÆ objet JS
// ---------------------------------------------------------------------------
void MonitorWorker::OnProgress(const JobEvent* data, size_t count) {
  Napi::Env env = env_; // Utilisation de l'env stock├®
...
Napi::Value PrintJob(const Napi::CallbackInfo& info) {
  // Simplifi├® pour la PR, ├á r├®-impl├®menter si n├®cessaire pour l'app
  return Napi::Boolean::New(info.Env(), true);
}

// ---------------------------------------------------------------------------
// ReanalyzeJob : Retourne les m├®tadonn├®es brutes d'un job par son ID
// ---------------------------------------------------------------------------
Napi::Value ReanalyzeJob(const Napi::CallbackInfo& info) {
  ...
  // Worker temporaire (non lanc├®) pour r├®utiliser les helpers SPL.
  ...
  // Mais si le but est juste la re-g├®n├®ration via SPL, splPath suffit.
